...
namespace DAB2
{
    public class Dab : IDab
    {
    	...
    	public void retrait(DateTime dt, float montant)
    	{
    		...
    		foreach(var t in compte.getTransactions())
    		{    		
    			//On ne regarde que les 30 derniers jours	
    			//et les transactions negatives pour le calcul du plafond
    			if(Math.Abs(dt.Subtract(t.getDate()).Days) < 30  &&  t.getValeurTransaction() < 0) 
    			{
    				montantUtilise -= t.getValeurTransaction(); 
    			}
    		} 
    		
    		if(montantUtilise + montant > compte.getMontantMax())
    		{
    			Console.WriteLine(dt.ToString("d") + " : Le montant maximum disponible de (" + (compte.getMontantMax() - montantUtilise) + " euros) est insuffisant pour le retrait de " + montant + " euros.");
    			return;
    		}
    		else {
    			compte.addTransaction(dt, -montant);
